...
class Treap<T = number> {
  ...
  constructor(compareFn: (a: T, b: T) => number = (a: T, b: T) => a < b ? -1 : (a > b ? 1 : 0), T_MIN: T = '__T_MIN__' as unknown as T, T_MAX: T = '__T_MAX__' as unknown as T) {
    ...
    this.compareFn = (a: T, b: T) => {
      if (a === this.T_MAX && b === this.T_MAX) return 0
      if (a === this.T_MIN && b === this.T_MIN) return 0
  ...
  *inOrder(node: TreapNode<T> | null = this.root): Generator<T, undefined, void> {
    if (!node) return
    for (const v of this.inOrder(node.left)) yield v
    if (this.compareFn(node.value, this.T_MAX) !== 0 && this.compareFn(node.value, this.T_MIN) !== 0) {
      for (let i = 0; i < node.cnt; i++) yield node.value
    }
    for (const v of this.inOrder(node.right)) yield v
  }
